...
class Copywriter:
  """scout 키워드·risk를 반영한 카피·상품명을 생성한다."""
  ...
  def build_context(self, keyword_ctx: dict[str, Any], product_title: str) -> dict[str, str]:
    ...
    if self.gemini.available:
      prompt = (
        ...
        f"키워드: {seed}, 연관: {related_str}. 과장·의료효능 금지."
      )
    ...
    return {
      "product_title": seed if len(seed) <= 40 else seed[:40],
      "hook_line": hook,
  ...
  def coupang_title(self, seed: str) -> str:
    """쿠팡 상품명 — title_max_chars 이내, 핵심 키워드 유지."""
    title = seed.strip()
    if len(title) <= self.title_max_chars:
      return title
    return title[: self.title_max_chars]
